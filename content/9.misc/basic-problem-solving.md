...
This page focuses mostly on functionality which is provided by IntelliJ (and many other IDEs) such as Debugging and Logging.

## Console and LOGGER

The most basic but also fastest tool to identify problems is the console. Values can be printed there at "run-time" which can inform the developer about the current sate of the code and makes it easy to analyze changes and potential mistakes.

In the `ModInitializer` implementing entrypoint class of the mod, a `LOGGER` is used to print the desired output to the console.
...
Keep in mind that this will also be printed if the mod is used in any other environment. This is completly optional, but i like to create a custom `LOGGER` method and use that, instead of the `LOGGER` itself to prevent printing data, which is only needed in a development environment.
...
A more "sophisticated" way of debugging is the usage of Breakpoints in an IDE. As their name suggests, they are used to halt the executed code at specific locations and make it possible to inspect and modify the state of the mod with a variety of tools.
...
```java
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        // get a list of the itemStack's enchantments as a Map
...
![runServer Gradle Task](/misc/basic_problem_solving_09.png)
...
![client 401 error status](/misc/basic_problem_solving_10.png)
...
![server prompts](/misc/basic_problem_solving_11.png)

To join the server, use `localhost` as the `Server adress` in-game.

### Set up the Client profile

Some testing requires multiple clients to interact in a multiplayer environment. Edit the `Minecraft Client` run configuration by pressing the white arrow.

![edit client configuration](/misc/basic_problem_solving_12.png)

And modify the options to run multiple instances of this profile at the same time.

![allow multiple instances](/misc/basic_problem_solving_13.png)
...
![consoles](/misc/basic_problem_solving_14.png)

## Logs and crash report files

The console of a previously executed instance helps with finding the issues. They are exported into the log files, located in the Minecraft instance's `logs` directory. The newest log is usually called `latest.log`. User can send this file, for further inspection, to the mod developer or host the file content on code-hosting websites.

In the developer environment you can find the logs in the project's `run > logs` folder and the crash reports in the `run > crash-reports` folder.
...
- [KaupenJoe's Youtube](https://www.youtube.com/@ModdingByKaupenjoe)
